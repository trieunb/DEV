...
function Ympicker() {
	...
	this._mainDivId = 'ui-monthpicker-div'; // The ID of the main ympicker division
...
//							$.ympicker._selectDay(event.target, inst.selectedMonth, inst.selectedYear, sel[0]);
	...
	_showYmpicker: function(input) {
		...
		//Duy fixed: get element width
		var parent = input.parentNode.getElementsByClassName('month');
		var inputWidth = parent[0].offsetWidth;
		//--------------------------------
		...
		inst.dpDiv.css({position: ($.ympicker._inDialog && $.blockUI ?
			...
			left: (offset.left - inputWidth - 5) + 'px', top: (offset.top + 24) + 'px'});
		if (!inst.inline) {
...
//							inst.id + '\',' + printDate.getMonth() + ',' + printDate.getFullYear() + ', this);return false;"') + '>' + // actions
							...
							(printDate.getYear() + printDate.getMonth() == today.getYear() + today.getMonth() ? ' ui-state-highlight' : '') +
...
//						printDate.setDate(printDate.getDate() + 1);
		...
		html += buttonPanel + (jQuery.support.msie && parseInt($.browser.version,10) < 7 && !inst.inline ?
