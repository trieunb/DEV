...
/**
*|--------------------------------------------------------------------------
*| Manufactor Instruction Search Export
*|--------------------------------------------------------------------------
*| Package       : Apel
...
class ManufacturingInstructionSearchExportController extends Controller
{
    public $title           = 'Manufactor Instruction Search';
    public $company         = 'Apel';
    public $description     = '製造指示書一覧';
    ...
    /*
    * post Manufacturing Instruction Search Export
    * -----------------------------------------------
    * @author      :   ANS796 - 2018/04/10 - create
    * @param       :
    * @return      :
    * @access      :   public
    * @see         :   remark
    */
    public function postManufacturingSearchOutput(Request $request) {
        try {
            $param          = \Input::all();
                    ...
                    $excel->sheet('Sheet 1', function($sheet) use ($result) {
                        ...
                        foreach ($result[0] as $k1 => $v1) {
                            ...
                            $sheet->row($row, array(
                                $v1['manufacture_no'],
                                $v1['in_order_no'],
                                $v1['internal_ordering_date'],
                                $v1['production_instruction_date'],
                                $v1['hope_delivery_date'],
                                $v1['outsourcing_status'],
                                $v1['orderer_cd'],
                                $v1['orderer_nm'],
                                $v1['processing'],
                                $v1['product_cd'],
                                ...
                                $v1['serial_no_from'],
                                $v1['serial_no_to'],
                                $v1['production_status'],
                                $v1['manufacturing_completion_date'],
                                $v1['required_calc_datetime'],
                            ));
                            ...
                            $sheet->cells('C'.$row.':E'.$row, function($cells) {
                                $cells->setAlignment('center');
                                $cells->setValignment('center');
                            });
                            ...
                            $sheet->cells('L'.$row.':M'.$row, function($cells) {
                                $cells->setAlignment('right');
                                $cells->setValignment('center');
                            });     
                            ...
                            $sheet->cells('O'.$row.':P'.$row, function($cells) {
                                $cells->setAlignment('center');
                                $cells->setValignment('center');
                            });
                        }
    ...
    * @see         :   remark
    */
    public function postManufacturingSearchExport(Request $request) {
        try {
            $param                     = $request->all();
            $param['update_list']      = json_encode($param['update_list']);//parse json to string
            $param['cre_user_cd']      = \GetUserInfo::getInfo('user_cd');
            $param['cre_prg_cd']       = '027_manufacturing-instruction-search';
            $param['cre_ip']           = \GetUserInfo::getInfo('user_ip');
            $sql                       = "SPC_027_MANUFACTURING_INSTRUCTION_EXPORT_ACT1";
            $result                    = Dao::call_stored_procedure($sql, $param,true);
            $response                  = true;
            $error                     = '';
            $error_cd                  = '';
            ...
            if (!empty($result[0])) {
                ...
                if (isset($result[1]) && !empty($result[1][0]['error_cd'])) {
                    ...
                    for ($k = 0; $k < count($result[2]); $k++) {
                        ...
                        // get data by key
                        $key            = ['manufacture_no' => $result[2][$k]['manufacture_no']];
                        ...
                        *********************************************************************/
                        ...
                        if ( !is_null($row_data)) {
                            $filename    = '製造指示書_'.$key['manufacture_no'];
