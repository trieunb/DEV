...
/**
*|--------------------------------------------------------------------------
*| Provisional Shipment Export
*|--------------------------------------------------------------------------
*| Package       : Apel
*| @author       : ANS831 - khadv@ans-asia.com
...
class ProvisionalShipmentSearchExportController extends Controller
{
    ...
    * @see         :   remark
    */
    public function postProvisionalShipmentOutput(Request $request) {
        try {
            $param          = \Input::all();
            $sql            = "SPC_008_PROVISIONAL_SHIPMENT_SEARCH_FND1";    //name stored
            ...
            if ( !is_null($result[0])) {
                ...
                \Excel::create($filename, function($excel) use ($result, $arrWidthColumns) {
                    $excel->sheet('Sheet 1', function($sheet) use ($result, $arrWidthColumns) {
                        ...
                        $sheet->getStyle('A1:K1')->applyFromArray($styleAllBorder);
                        ...
                        foreach ($result[0] as $k1 => $v1) {
                            $row ++;
                            $sheet->getStyle('A'.$row.':K'.$row)->applyFromArray($styleAllBorder);
                            $sheet->getStyle('A'.$row.':K'.$row)->getAlignment()->setWrapText(true);
                            $sheet->row($row, array(
                                $v1['deposit_no'], 
                                $v1['fwd_no'], 
                                $v1['fwd_detail_no'], 
                                $v1['lib_val_nm_j_forwarding_div'], 
                                $v1['cust_nm'],                                
                                $v1['lib_val_nm_j_country'], 
                                $v1['lib_val_nm_j_city'],
                                $v1['item_nm_j'],
                                $v1['fwd_qty'],
                                $v1['lib_val_nm_j_unit_q_div'],
                                $v1['gross_weight'],                                
                                // $v1['inside_remarks'],                                
                            ));
    ...
    */
    public function postExportExcel(Request $request) {
        try {
            // $data_request   =   $request->shipment_list;
            $zipFileName            =   '仮出荷指示書_'.date("YmdHis").'.zip';
            $error_flag             =   false;
            $data                   = $request->all();
            $data['fwd_list']       =  json_encode($data['fwd_list']);//parse json to string
            $data['cre_user_cd']    = \GetUserInfo::getInfo('user_cd');
            $data['cre_prg_cd']     = '007_provisional_shipment-excel';
            $data['cre_ip']         = \GetUserInfo::getInfo('user_ip');
            $sql                    =   "SPC_009_PROVISIONAL_SHIPMENT_DETAIL_EXCEL_ACT1"; 
            ...
            if (!empty($result[0])) {
                ...
                *********************************************************************/
                    for ($k = 0; $k < count($result[2]); $k++) {
                        ...
                        $data_pagi      = dataPageExcel($row_data,$page);        
                        // if ( !is_null($header)) {
                            $filename    = '仮出荷指示一覧_'.$header['fwd_no'];
                            // return $filename;
                            \Excel::create($filename, function($excel) use ($data_pagi, $header, $line_detail, $arrWidthColumns, $serial, $marginPage) {
                                $excel->sheet('Sheet 1', function($sheet) use ($data_pagi, $header, $line_detail, $arrWidthColumns, $serial, $marginPage) {
                                    ...
                                    //set title
                                    $posMerg    =  $pos + 1;
                                    $sheet->setCellValue('G'.$pos, '出荷指示書（仮）');
                                    ...
                                    //     $cells->setValignment('center');   
                                    // });
                                    // $sheet->getStyle('R'.$pos_rcv_no)->getBorders()->getLeft()->setBorderStyle(\PHPExcel_Style_Border::BORDER_MEDIUM, "#000000");
                                    ...
                                    // $sheet->getStyle('R'.$pos_pf_inv_no)->getBorders()->getLeft()->setBorderStyle(\PHPExcel_Style_Border::BORDER_MEDIUM, "#000000");
                                    ...
                                    foreach ($data_pagi[$i] as $k => $v) {
                                            ...
                                            */
                                            ...
                                            // $sheet->getStyle('A'.$row.':E'.$row)->getBorders()->getLeft()->setBorderStyle(\PHPExcel_Style_Border::BORDER_DOTTED, "#000000");

                                            //set height  品名
                                            $height             =   numLineOfRowExcel($v['item_nm_j'], 28);
                                            $total_line_detail  =   $total_line_detail + $height;
                                            ...
                                            // $sheet->setBorder('H'.$row.':Q'.$row, \PHPExcel_Style_Border::BORDER_DOTTED, "#000000");
                                            // Increment auto 'row'
                                            $row++;
                                        }
                                        ...
                                        $line   =   0;
                                        if ($line_detail > $total_line_detail) {
                                            $line = $line_detail - $total_line_detail;
                                            for ($j = 0; $j < $line; $j ++) { 
                                                $posNull    =   $posNull + 1;
                                    ...
                                    $sheet->setCellValue('A'.$pos_purchasing, '');
                                    ...
                                    $sheet->setCellValue('E'.$pos_purchasing, '');
                                    ...
                                    $sheet->setCellValue('A'.$pos_confirm, '');
        ...
        try {
            ...
            if (!empty($data)) {
                foreach ($data as $key => $value) {
                    $data_Detail[] =  [['value' => $value['item_nm_j'] , 'leng' => '28']];
                }
            }
            return $data_Detail;
        } catch (\Exception $e) {
