			...
			<div class="table-responsive table-custom ">
				<table class="table table-hover table-bordered table-xxs table-list table-invoice tablesorter" id="table-invoice">
					...
					<tr class="col-table-header text-center">
						...
						<!-- inv_data_div -->
						<th class="text-center" width="8%">区分</th>
						<!-- inv_date -->
						...
						<!-- rcv_no -->
						<th class="text-center" width="9%">受注No</th>
						<!-- pi_no -->
						<th class="text-center" width="10%">PINo</th>
						<!-- cust_nm -->
						...
						@if(isset($invoiceList) && !empty($invoiceList))
							@foreach($invoiceList as $invoice)
								<tr class="tr-table">
									...
									<!-- inv_date -->
									<td class="text-center">{{$invoice['inv_date'] or ''}}</td>

									<!-- inv_no -->
									<td class="text-left inv_no">{{$invoice['inv_no'] or ''}}</td>

									<!-- inv_detail_no -->
									<td class="text-left">{{$invoice['inv_detail_no'] or ''}}</td>

									<!-- rcv_no -->
									<td class="text-left">{{$invoice['rcv_no'] or ''}}</td>

									<!-- pi_no -->
									<td class="text-left">{{$invoice['pi_no'] or ''}}</td>
